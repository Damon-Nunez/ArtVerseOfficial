    ...
    try {
        ...
        if(results.rows.length === 0) {
            ...
            if (result.rows.length === 1) {
              return NextResponse.json({ message: 'You have already favorited this post.' }, { status: 400 });
}

    const insertFavorite =
    `
    INSERT INTO favorites (artist_id, post_id)
    VALUES($1,$2)
    ON CONFLICT (artist_id, post_id) DO NOTHING
    RETURNING *
    `;

    const insertResult = await pool.query(insertFavorite, [artist_id,post_id]);

         if (insertResult.rows.length > 0) {
        return NextResponse.json({ message: 'Post favorited successfully!' }, { status: 201 });
    } else {
        return NextResponse.json({ message: 'Failed to favorite post.' }, { status: 500 });
    }  
